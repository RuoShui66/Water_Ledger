...
让 Agent 初始化默认账户，然后导入支付宝 CSV、微信 Excel 和银行卡 PDF。每笔交易会归到实际出钱的账户：银行卡支付影响银行卡余额，微信余额/支付宝余额支付影响对应钱包余额；支付宝/微信账单还会提供更清楚的商户、商品和备注。

```text
我先只记日常收支。请初始化默认账户，并告诉我把支付宝、微信、银行卡账单放哪里。
```

### 我还有更多账户

在会话里直接告诉 Agent 要加什么账户。Agent 应该弹出对话框问账户类型、币种、是否计入净资产和当前余额。

```text
帮我加一张招商银行卡。
我还有一个美元券商账户。
加一个信用卡欠款账户，不计入资产，只作为负债。
```

### 我想回溯券商历史净资产

新增券商账户后，可以让 Agent 运行批量拉取脚本，回溯你选择时间范围内的历史净资产：

```text
我新增了一个券商账户，请帮我拉取 2025-01-01 到今天的历史净资产并导入。
```

Agent 会处理脚本配置、批量拉取、导入和看板刷新。前端资产曲线会按拉取到的日期显示历史净资产。如果某个券商暂时没有可用脚本，Agent 会再告诉你可以提供什么格式的历史数据作为兜底。

新增券商账户后，Agent 也应该顺手安排之后每天自动更新净资产；Codex app 默认可以用 Codex automation，Claude Code 默认可以用 Claude routine/cron。没有 Agent 原生定时能力时，再使用项目本地定时任务。

### 我只有今天的券商余额

可以先让 Agent 录当前余额，作为今天的资产快照；以后再补历史数据。只录当前余额不能还原过去每天的市场波动。

### 我想以后每天自动更新券商净资产

让 Agent 帮你打开每日自动更新。Agent 会安排一个每日任务，到点自动拉取已启用券商账户的净资产，并写入本地数据库和看板。

```text
请帮我打开每日券商净资产更新，时间设为每天 04:01。
```

在 Codex app 里，默认可以用 Codex 自带的 automation；在 Claude Code 里，默认可以用 Claude 的 routine/cron。你可以直接这样说：

```text
请每天 04:01 在这个项目里运行券商净资产快照。
任务内容：进入当前 Water Ledger 项目，运行 .venv/bin/python -m water_ledger brokerage-snapshot --provider enabled；
如果没有 .venv，就用 python -m water_ledger brokerage-snapshot --provider enabled。
```

在手动终端或没有 Agent 原生定时能力的环境里，Agent 会使用项目本地定时任务：macOS 用 LaunchAgent，Windows 用任务计划程序。两种方式最终都会运行同一个 `brokerage-snapshot` 项目命令。
