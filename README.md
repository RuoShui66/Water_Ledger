...
新增券商账户后，可以让 Agent 运行批量拉取脚本，回溯任意时间段的历史净资产，不限定半年：

```text
我新增了一个券商账户，请帮我拉取 2025-01-01 到今天的历史净资产并导入。
```

Agent 会配置或运行 `brokerage-history`，把脚本拉到的历史快照写入 `private/imports/brokerage/`，然后重建账本：
...
前端资产曲线会按脚本返回的日期显示历史净资产。CSV 只是内部导入格式或兜底方式，用户不需要手动整理上传。

如果某个券商暂时没有可用脚本，Agent 可以再退回到 CSV 导入。

### 我只有今天的券商余额

可以先录当前余额，作为今天的资产快照；以后再补历史 CSV。只录当前余额不能还原过去每天的市场波动。
