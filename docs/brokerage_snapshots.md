...
- 批量拉取历史净资产：适合回溯你选择的时间范围，例如过去 3 个月、今年以来或完整开户以来。
- 调用券商实时接口：适合记录今天或定期记录之后的净资产。
- 安装本地每日任务：适合之后每天自动更新券商净资产曲线。
...
Water Ledger 支持两类每日更新方式：

- Agent 原生定时任务：Codex app 用 automation，Claude Code 用 routine/cron，适合用自然语言让 Agent 每天唤醒执行。
- 项目本地定时任务：适合手动终端、Windows/macOS 本机调度，macOS 使用 LaunchAgent，Windows 使用任务计划程序。
...
```text
private/logs/brokerage_snapshot.log
```
...
Windows 会生成私有 wrapper：

```text
private/logs/daily_brokerage_snapshot.cmd
```

调度元数据写入：

```text
private/logs/brokerage_schedule.json
```

如果只想生成配置给用户检查，不立即加载到系统，可以加 `--write-only`。

### Agent 原生定时任务

在 Codex app 或 Claude Code 里，用户可以直接让 Agent 创建每日任务。推荐自然语言：
...
Codex app 应创建 automation；Claude Code 应创建 routine/cron。手动用户或没有 Agent 原生定时能力的环境应使用 `brokerage-schedule`。
